...
static void linlondp_wb_job_timer_callback(struct timer_list *t)
{
	...
	job = list_first_entry_or_null(&wb_conn->base.job_queue,
				...
				list_entry);
	if (job) {
		drm_dbg_atomic(kcrtc->base.dev, "Timer fired! has JOB not signal\n");
				   ...
				   struct drm_crtc_state *old)
{
	...
	drm_dbg_atomic(crtc->dev, "CRTC%d_FLUSH: active_pipes: 0x%x, affected: 0x%x.\n",
					...
					struct drm_atomic_state *state)
{
	...
	if (old && old->self_refresh_active) {
		drm_dbg_atomic(mdev->drm, "%s self_refresh_active, do nothing.\n", __func__);
					 ...
					 struct drm_atomic_state *state)
{
	...
	drm_dbg_atomic(mdev->drm, "CRTC%d_DISABLE: active_pipes: 0x%x, affected: 0x%x\n",
			 ...
			 const struct drm_display_mode *m)
{
	...
	if (min_pxlclk > clk_round_rate(master->pxlclk, min_pxlclk)) {
		drm_dbg_atomic(kcrtc->base.dev, "pxlclk doesn't support %lu Hz\n", min_pxlclk);
				     ...
				     struct drm_display_mode *adjusted_mode)
{
	...
	if (rebuilt_crtcs) {
		...
		drm_dbg_atomic(mdev->drm, "%s sidy_by_side. rebuild crtcs",
