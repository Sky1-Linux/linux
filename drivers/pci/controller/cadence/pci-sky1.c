...
/*
 * Mainline doesn't export cdns_pcie_host_restore().
 * We use cdns_pcie_host_link_setup() + cdns_pcie_host_init() instead.
 */
static int sky1_pcie_host_restore(struct cdns_pcie_rc *rc)
{
	int ret;
...
static int sky1_pcie_resume_noirq(struct device *dev)
{
	...
	ret = sky1_pcie_host_restore(rc);
	if (ret < 0)
		dev_err(dev, "sky1_pcie_host_restore err, err=%d\n", ret);
...
MODULE_AUTHOR("CIX Technology");
MODULE_DESCRIPTION("CIX Sky1 PCIe host controller driver");
MODULE_LICENSE("GPL");
