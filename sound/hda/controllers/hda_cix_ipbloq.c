...
// Ported to mainline 6.18 for Radxa Orion O6
...
static int cix_ipbloq_hda_send_init_verbs(struct cix_ipbloq_hda *hda)
{
	...
	/* Select verb table based on board model */
	...
	} else if (hda->sname && !strcmp(hda->sname, "CIX SKY1 ORAPI 6P HDA")) {
	...
	if (!init_verbs || !size) {
		dev_warn(bus->dev, "no init verbs for model: %s\n",
			 hda->sname ? hda->sname : "unknown");
		return 0;
	}
...
static void cix_ipbloq_hda_probe_work(struct work_struct *work)
{
	...
	/* Send init verbs before codec probe */
				 ...
				 struct cix_ipbloq_hda *hda)
{
	...
	/*
	 * Note: DMA address translation is handled via dma-ranges in device tree.
	 * The HDA controller sees memory at a 0x90000000 offset from CPU addresses.
	 * This is configured in sky1.dtsi via the hda_bus sub-bus dma-ranges property.
	 */
...
static int cix_ipbloq_hda_probe(struct platform_device *pdev)
{
	...
	err = snd_card_new(&pdev->dev, SNDRV_DEFAULT_IDX1, SNDRV_DEFAULT_STR1,
			   THIS_MODULE, 0, &card);
	if (err < 0) {
		dev_err(&pdev->dev, "failed to create card, err = %d\n", err);
		return err;
	}
